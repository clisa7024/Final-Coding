...
public class LoanCalcViewController implements Initializable {
	...
	private boolean ValidateData() {
		...
		if(LoanAmount.getText().isPositive()) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Not a number");
			fail.setContentText("Loan Amount is not positive");
			fail.showAndWait();
			return false;
		}
		// TODO: Validate InterestRate is not blank and is a positive double. Validate
		if(InterestRate.getText().isPositive() && InterestRate.getText().trim().isEmpty()) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Missing Data or not a number");
			fail.setContentText("Loan Amount is required or is not positive");
			fail.showAndWait();
			return false;
		}
		// that the value is between 1 and 30 (2.99 = 2.99%, not 0.0299)
		// TODO: Validate NbrOfYears is a non blank positive integer
		if(NbrOfYears.getText().isPositive() && NbrOfYears.getText().trim().isEmpty()) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Missing Data or not a number");
			fail.setContentText("Loan Amount is required or is not positive");
			fail.showAndWait();
		...
		if(AddtionalPayment.getText().isPositive()) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Not a number");
			fail.setContentText("Loan Amount is not positive");
			fail.showAndWait();
		...
		if(ExcrowAmount.getText().isPositive()) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Not a number"");
			fail.setContentText("Loan Amount is not positive");
			fail.showAndWait();
	...
	private void btnCalcLoan(ActionEvent event) {
		...
		lblInterestSaved.setText(fmtCurrency.format(loanExtra.getInterestSaved()));
		lblPaymentsSaved
